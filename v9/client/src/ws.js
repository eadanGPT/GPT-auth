...
import { createEncryptedExpression, initProtect, protectString } from './runtime_protect.js';
import { getPermanentHWID, getTemporaryHWID } from './hwid.js';
import { config } from './config.js';

export async function connectAndAuth(licenseKey) {
  const clientKeyHash = deriveClientKeyHash(licenseKey);
  initProtect(clientKeyHash);
  ...
  function exitNow(){ if (ws) try { ws.close(); } catch {} process.exit(1); }
    ...
    ws.on('message', (raw)=>{
      ...
      if (msg.type === 'hello') {
        ...
        const info = {
          ...
          hwid_perm: getPermanentHWID(),
          temp_hwid: getTemporaryHWID(),
        };
        ...
        if (!dec.ok) exitNow();
        jwtToken = dec.token;
        // Protect in-memory token
        createEncryptedExpression(jwtToken);
        resolve();
      } else if (msg.type === 'hb') {
        ...
        // ignore, could upload manifests here
      }
    });
    ws.on('close', ()=>{
      ...
      if (retries < 1) {
        retries++;
        setTimeout(()=>connectAndAuth(licenseKey).catch(()=>process.exit(1)), 15000);
      } else {
        process.exit(1);
      }
    });
  ...
  return { ws, send: (obj)=>{
    const payload = Buffer.from(JSON.stringify({ token: jwtToken, kind:'data', data: obj }));
    const out = aesGcmEncrypt(sessionKey, sessionIv, payload);
    ws.send(JSON.stringify({ type:'packet', data: out.toString('base64') }));
  }};
}
